...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"Inline";
    ...
    if (UIInterfaceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation)) {
        adView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, screenBounds.size.height - 50.0f - statusBarHeight, screenBounds.size.width, 50.0f)];
    }
    else {
        adView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, screenBounds.size.width - 50.0f - statusBarHeight, screenBounds.size.height, 50.0f)];
    }
...
- (void) initRemon {
    ...
    [core setUseGioTargeting:YES];                                      //Local Tagerting 사용 여부 설정 (optional, default : NO)
    [core setIsAgreeTerm:YES];                                          //약관 동의 여부 설정 (optinal, default : NO)
    [core setClientID:@"MTQyODkzNTM5OTcyNzM5"];                         //발급 받은 cId로 대치 (Mandatory)
    [core setDefaultBGColor:[UIColor blueColor]];                       //BG color 설정 (optional, default : whiteColor 사용자가 설정한 값이 없을 경우 광고에서 설정된 값이 노출된다.)
