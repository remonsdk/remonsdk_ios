...
- (void)viewDidLoad {
    ...
    if (UIInterfaceOrientationIsPortrait([UIApplication sharedApplication].statusBarOrientation)) {
        adView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, screenBounds.size.height - 250.0f - statusBarHeight, screenBounds.size.width, 250.0f)];
    }
    else {
        adView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, screenBounds.size.width - 250.0f - statusBarHeight, screenBounds.size.height, 250.0f)];
    }
...
- (void) initRemon {
    ...
    [core setClientID:@"MTQyODkzNTQwNzcyNzQw"];                         //발급 받은 cId로 대치 (Mandatory)
    [core setIsAgreeTerm:YES];                                          //약관 동의 여부 설정 (optinal, default : NO)
    [core setDefaultBGColor:[UIColor redColor]];                        //BG color 설정 (optional, default : whiteColor 사용자가 설정한 값이 없을 경우 광고에서 설정된 값이 노출된다.)
    [core setRefreshInterval:30];                                       //광고 갱신 시간 설정 (optional, default : 30, 범위 : 30 ~ 300초)
